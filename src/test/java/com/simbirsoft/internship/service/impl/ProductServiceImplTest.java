...
import static com.simbirsoft.internship.testdata.ProductTestData.*;
...
class ProductServiceImplTest {
    ...
    void findAllById() {
        ...
        when(productRepository.findAllById(ids)).thenReturn(Arrays.asList(PROD_ENTITY_1, PROD_ENTITY_2));
        // when
        List<ProductEntity> result = productService.findAllById(ids);
        // than
        assertIterableEquals(Arrays.asList(PROD_ENTITY_1, PROD_ENTITY_2), result);
    ...
    void createNewInvoice() {
        // given
        when(productRepository.saveAll(anyCollection())).thenReturn(Arrays.asList(PROD_ENTITY_1, PROD_ENTITY_2));
        ...
        // when
        List<ProductEntity> result = productService.createList(Arrays.asList(PROD_DTO_1, PROD_DTO_2));
        // than
        assertIterableEquals(Arrays.asList(PROD_ENTITY_1, PROD_ENTITY_2), result);
        verify(productRepository, times(1)).saveAll(anyCollection());
        verify(productRepository, times(2)).findByName(anyString());
    ...
    void createNewInvoice_CatNotFound() {
        ...
        when(categoryRepository.findById(anyInt())).thenReturn(java.util.Optional.of(CAT_1));
        // when
        assertThrows(InvalidPropertyException.class, () -> productService.createList(Arrays.asList(PROD_DTO_INVALID_CATID, PROD_DTO_2)));
        // than
        verifyNullTimes();
    }
    ...
    void createInvoice_UpdateInvalidNameProducts() {
        // when
        assertThrows(InvalidPropertyException.class, () -> productService.createList(Arrays.asList(PROD_DTO_INVALID_NAME, PROD_DTO_2)));
        // than
        verifyNullTimes();
    }
    ...
    void createInvoice_UpdateInvalidDescProducts() {
        // when
        assertThrows(InvalidPropertyException.class, () -> productService.createList(Arrays.asList(PROD_DTO_INVALID_DESC, PROD_DTO_2)));
        // than
        verifyNullTimes();
    }
    ...
    void createInvoice_UpdateInvalidAmountProducts() {
        // when
        assertThrows(InvalidPropertyException.class, () -> productService.createList(Arrays.asList(PROD_DTO_INVALID_AMOUNT, PROD_DTO_2)));
        // than
        verifyNullTimes();
    }
